namespace GosuMechanicsYasuo
{
    ...
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config.SubMenu("Escape").AddItem(new MenuItem("flee", "Escape")).SetValue(new KeyBind('Z', KeyBindType.Press));
            Config.SubMenu("Escape").AddItem(new MenuItem("wall", "WallJump Escape")).SetValue(new KeyBind('V', KeyBindType.Press));
        ...
        private static void Obj_AI_Base_OnPlayAnimation(Obj_AI_Base sender, GameObjectPlayAnimationEventArgs args)
        {
            ...
            isDashing = true;
            Utility.DelayAction.Add(300, () => { if (Me.IsDashing()) { isDashing = false; } });
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            ...
            if (!IsDashing && Config.Item("AutoQHarass").GetValue<KeyBind>().Active && Orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.Combo)
            {
                ...
                else if (TsTarget != null && !Config.Item("HarassTower").GetValue<bool>() && TsTarget.CharData.BaseSkinName != "gangplankbarrel" && !UnderTower(Me.ServerPosition.To2D()) && Q3.IsReady() && Config.Item("HarassQ3").GetValue<bool>() && !IsDashing && Q3READY() && Q3.IsInRange(TsTarget))
                {
                    CastQ3(TsTarget);
                }
            }
            ...
            if (!IsDashing && Config.Item("AutoQHarass").GetValue<KeyBind>().Active && Orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.Combo)
            {
                ...
                else if (TsTarget != null && TsTarget.CharData.BaseSkinName != "gangplankbarrel" && !Config.Item("HarassTower").GetValue<bool>() && !Q3READY() && Q.IsReady() && Config.Item("HarassQ").GetValue<bool>() && !IsDashing && !UnderTower(Me.ServerPosition.To2D()) && Q.IsInRange(TsTarget))
                {
                    CastQ12(TsTarget);
                }
            }
        ...
        {
            foreach (var minion in MinionManager.GetMinions(Me.ServerPosition, Q.Range).OrderByDescending(m => m.Health))
            {
                if (minion != null)
                {
                    if (!minion.IsDead && Config.Item("LastHitQ1").GetValue<bool>() && Q.IsReady() && minion.IsValidTarget(500) && !Q3READY() && Q.IsInRange(minion))
                    {
                        var predHealth = HealthPrediction.GetHealthPrediction(minion, (int)(Me.Distance(minion.Position) * 1000 / 2000));
        ...
        public static void Harass()
        {
            ...
            if (Config.Item("HarassTower").GetValue<bool>())
            {
            ...
            else if (!Config.Item("HarassTower").GetValue<bool>())
            {
                if (!UnderTower(Me.ServerPosition.To2D()) && Q3.IsReady() && Config.Item("HarassQ3").GetValue<bool>() && !IsDashing && Q3READY() && Q3.IsInRange(TsTarget))
                {
                    CastQ3(TsTarget);
                }
                if (!Q3READY() && Q.IsReady() && Config.Item("HarassQ").GetValue<bool>() && !IsDashing && !UnderTower(Me.ServerPosition.To2D()) && Q.IsInRange(TsTarget))
                {
                    CastQ12(TsTarget);
                }
            }
        ...
        {
            var Qminion = MinionManager.GetMinions(ObjectManager.Player.ServerPosition, 1000, MinionTypes.All, MinionTeam.NotAlly);
        ...
        {
            foreach (var minion in MinionManager.GetMinions(Me.ServerPosition, Q3.Range).OrderByDescending(m => m.Health))
            {
                if (minion == null)
                {
                    return;
                ...
                if (!minion.IsDead && Config.Item("LastHitQ1").GetValue<bool>() && Q.IsReady() && minion.IsValidTarget(500) && !Q3READY() && Q.IsInRange(minion) && !IsDashing)
                {
                    var predHealth = HealthPrediction.GetHealthPrediction(minion, (int)(Me.Distance(minion.Position) * 1000 / 2000));
                    if (predHealth <= GetQDmg(minion))
                    {
                ...
                if (!minion.IsDead && Config.Item("LastHitQ3").GetValue<bool>() && Q.IsReady() && minion.IsValidTarget(1100) && Q3READY() && Q3.IsInRange(minion) && !IsDashing)
                {
                    var predHealth = HealthPrediction.GetHealthPrediction(minion, (int)(Me.Distance(minion.Position) * 1000 / 2000));
                    if (predHealth <= GetQDmg(minion))
                    {
        ...
        {
            var Qminion = MinionManager.GetMinions(ObjectManager.Player.ServerPosition, 1000);
            foreach (var minion in Qminion.Where(minion => minion.IsValidTarget(Q3.Range)))
            {
                ...
                if (Config.Item("LaneClearQ1").GetValue<bool>() && Q.IsReady() && minion.IsValidTarget(500) && !Q3READY() && Q.IsInRange(minion))
                {
                    var predHealth = HealthPrediction.LaneClearHealthPrediction(minion, (int)(Me.Distance(minion.Position) * 1000 / 2000));
                    ...
                    {
                        List<Vector2> minionPs = GetCastMinionsPredictedPositions(Qminion, .025f, 50f, float.MaxValue, Me.ServerPosition, 475f, false, SkillshotType.SkillshotLine);
                ...
                if (!minion.IsDead && Config.Item("LaneClearQ3").GetValue<bool>() && Q3.IsReady() && minion.IsValidTarget(1100) && Q3READY() && Q3.IsInRange(minion))
                {
                    var predHealth = HealthPrediction.LaneClearHealthPrediction(minion, (int)(Me.Distance(minion.Position) * 1000 / 2000));
                    if (predHealth <= GetQDmg(minion) && !IsDashing)
                    {
                    ...
                    {
                        var minionPs = GetCastMinionsPredictedPositions(Qminion, .025f, 50f, float.MaxValue, Me.ServerPosition, 1000f, false, SkillshotType.SkillshotLine);
                        var farm = Q3.GetLineFarmLocation(minionPs);
            ...
            var allMinionsE = MinionManager.GetMinions(Me.ServerPosition, E.Range);
            ...
            var jminions = MinionManager.GetMinions(Me.ServerPosition, 1000, MinionTypes.All, MinionTeam.Neutral);
            foreach (var jungleMobs in jminions.Where(x => x.IsValidTarget(Q3.Range)))
            {
                ...
                if (Config.Item("JungleClearE").GetValue<bool>() && E.IsReady() && jungleMobs.IsValidTarget(E.Range) && CanCastE(jungleMobs))
                {
                    E.CastOnUnit(jungleMobs);
                }
                if (Config.Item("JungleClearQ3").GetValue<bool>() && Q3.IsReady() && jungleMobs.IsValidTarget(1000) && Q3READY() && Q3.IsInRange(jungleMobs))
                {
                    CastQ3(jungleMobs);
                }
                if (Config.Item("JungleClearQ12").GetValue<bool>() && Q.IsReady() && jungleMobs.IsValidTarget(500) && !Q3READY() && Q.IsInRange(jungleMobs))
                {
                    CastQ12(jungleMobs);
                }
            }
        ...
        {
            if (Items.HasItem((int)ItemId.Blade_of_the_Ruined_King, Me) && Items.CanUseItem((int)ItemId.Blade_of_the_Ruined_King)
               && Config.Item("comboItems").GetValue<bool>() && Me.HealthPercent <= Config.Item("myHP").GetValue<Slider>().Value)
            {
                Items.UseItem((int)ItemId.Blade_of_the_Ruined_King, unit);
            }
            if (Items.HasItem((int)ItemId.Bilgewater_Cutlass, Me) && Items.CanUseItem((int)ItemId.Bilgewater_Cutlass)
               && unit.IsValidTarget(Q.Range))
            {
                Items.UseItem((int)ItemId.Bilgewater_Cutlass, unit);
            }
            if (Items.HasItem((int)ItemId.Youmuus_Ghostblade, Me) && Items.CanUseItem((int)ItemId.Youmuus_Ghostblade)
               && Me.Distance(unit.Position) <= Q.Range)
            {
                Items.UseItem((int)ItemId.Youmuus_Ghostblade);
            }
            if (Items.HasItem((int)ItemId.Ravenous_Hydra_Melee_Only, Me) && Items.CanUseItem((int)ItemId.Ravenous_Hydra_Melee_Only)
               && Me.Distance(unit.Position) <= 400)
            {
                Items.UseItem((int)ItemId.Ravenous_Hydra_Melee_Only);
            }
            if (Items.HasItem((int)ItemId.Tiamat_Melee_Only, Me) && Items.CanUseItem((int)ItemId.Tiamat_Melee_Only)
               && Me.Distance(unit.Position) <= 400)
            {
                Items.UseItem((int)ItemId.Tiamat_Melee_Only);
            }
            if (Items.HasItem((int)ItemId.Randuins_Omen, Me) && Items.CanUseItem((int)ItemId.Randuins_Omen)
               && Me.Distance(unit.Position) <= 400)
            {
        ...
        public static void AutoR()
        {
            ...
            if (enemies.Count >= autoREnemies && Me.Health >= MyHP && Me.CountEnemiesInRange(1500) <= enemyInRange)
            {
                R.Cast();
            }
        }
        public static void Combo()
        {
            ...
            if (Config.Item("QC").GetValue<bool>())
            {
                ...
                {
                    var QPred = Q.GetPrediction(TsTarget);
            ...
            if (Config.Item("smartW").GetValue<bool>() && wallCasted && Me.Distance(TsTarget.Position) < 300)
            {
                eBehindWall(TsTarget);
            }
            if (Config.Item("EC").GetValue<bool>())
            {
                var dmg = ((float)Me.GetSpellDamage(TsTarget, SpellSlot.Q) + (float)Me.GetSpellDamage(TsTarget, SpellSlot.E) + (float)Me.GetSpellDamage(TsTarget, SpellSlot.R));
                if (E.IsReady() && TsTarget.Distance(Me) >= (Config.Item("E1").GetValue<Slider>().Value) && dmg >= TsTarget.Health && UnderTower(PosAfterE(TsTarget)) && CanCastE(TsTarget) && Me.IsFacing(TsTarget))
                {
                    E.CastOnUnit(TsTarget);
                }
                else if(TsTarget.Distance(Me) >= (Config.Item("E1").GetValue<Slider>().Value) && dmg <= TsTarget.Health && CanCastE(TsTarget) && Me.IsFacing(TsTarget))
                {
                    useENormal(TsTarget);
                }
                else if (Q.IsReady() && IsDashing && Me.Distance(TsTarget) <= 275 * 275)
                {
                    Utility.DelayAction.Add(200, () => { CastQ12(TsTarget); } );
                }
                else if (Q3.IsReady() && Me.Distance(TsTarget) <= E.Range && Q3READY() && E.IsReady() && CanCastE(TsTarget))
                {
                    E.CastOnUnit(TsTarget, true);
                }
                else if (Q3.IsReady() && IsDashing && Me.Distance(TsTarget) <= 275 * 275 && Q3READY())
                {
                ...
                if (Config.Item("E3").GetValue<bool>() && E.IsReady())
                {
                    ...
                    ObjectManager.Get<Obj_AI_Base>()
                    .Where(x => x.IsValidTarget(E.Range))
                    .Where(x => x.Distance(TsTarget) < Me.Distance(TsTarget))
                    .OrderByDescending(x => x.Distance(Me))
                    .FirstOrDefault();
                    var dmg2 = ((float)Me.GetSpellDamage(TsTarget, SpellSlot.Q) + (float)Me.GetSpellDamage(TsTarget, SpellSlot.E) + (float)Me.GetSpellDamage(TsTarget, SpellSlot.R));
                    if (bestMinion != null && dmg2 >= TsTarget.Health && UnderTower(PosAfterE(bestMinion)) && Me.IsFacing(bestMinion) && TsTarget.Distance(Me) >= (Config.Item("E2").GetValue<Slider>().Value) && CanCastE(bestMinion) && Me.IsFacing(bestMinion))
                    {
                        E.CastOnUnit(bestMinion, true);
                    }
                    else if (bestMinion != null && dmg2 <= TsTarget.Health && Me.IsFacing(bestMinion) && TsTarget.Distance(Me) >= (Config.Item("E2").GetValue<Slider>().Value) && CanCastE(bestMinion) && Me.IsFacing(bestMinion))
                    {
                ...
                if (!Config.Item("E3").GetValue<bool>() && E.IsReady())
                {
                    ...
                    var dmg3 = ((float)Me.GetSpellDamage(TsTarget, SpellSlot.Q) + (float)Me.GetSpellDamage(TsTarget, SpellSlot.E) + (float)Me.GetSpellDamage(TsTarget, SpellSlot.R));
                    if (bestMinion != null && dmg3 >= TsTarget.Health && UnderTower(PosAfterE(bestMinion)) && Me.IsFacing(bestMinion) && TsTarget.Distance(Me) >= (Config.Item("E2").GetValue<Slider>().Value) && CanCastE(bestMinion) && Me.IsFacing(bestMinion))
                    {
                        E.CastOnUnit(bestMinion, true);
                    }
                    else if (bestMinion != null && dmg3 <= TsTarget.Health && Me.IsFacing(bestMinion) && TsTarget.Distance(Me) >= (Config.Item("E2").GetValue<Slider>().Value) && CanCastE(bestMinion) && Me.IsFacing(bestMinion))
                    {
                        useENormal(bestMinion);
                    }
                }
            ...
            if (R.IsReady() && Config.Item("R").GetValue<bool>())
            {
                ...
                if (Config.Item("Ignite").GetValue<bool>() && TsTarget.Health <= Me.GetSummonerSpellDamage(TsTarget, Damage.SummonerSpell.Ignite))
                {
                    Ignite.Cast(TsTarget);
                }
                
                if (Config.Item("comboItems").GetValue<bool>() && TsTarget.IsValidTarget())
                {
                    UseItems(TsTarget);
                }
            }
        ...
        public static void KillSteal()
        {
            foreach (var enemy in HeroManager.Enemies)
            {
                if (enemy.IsValidTarget(Q.Range) && Config.Item("KS").GetValue<bool>())
                {
                    ...
                    {
                        if (enemy.Health <= Me.GetSummonerSpellDamage(enemy, Damage.SummonerSpell.Ignite))
                        {
                            Ignite.Cast(enemy);
                        }
                    }
        ...
        {
            foreach (var target in HeroManager.Enemies.Where(x => x.IsValidTarget(1100)))
            {
                var Q3Pred = Q3.GetPrediction(target, true);
        ...
        {
            return Me.ServerPosition.Extend(target.ServerPosition, Me.Distance(target) < 410 ? E.Range : Me.Distance(target) + 65).To2D();
        }
        ...
        public static bool IsDashing => isDashing || Me.IsDashing();
        ...
        public static void putWallBehind(Obj_AI_Hero target)
        {
            ...
            float dist = Me.Distance(po.UnitPosition);
        ...
        public static void eBehindWall(Obj_AI_Hero target)
        {
            ...
            {
                foreach (var enemy in ObjectManager.Get<Obj_AI_Base>().Where(enemy => enemyIsJumpable(enemy)))
                {
        ...
        public static IsSafeResult isSafePoint(Vector2 point, bool igonre = false)
        {
            ...
            var safe = Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo ||
                        point.To3D().GetEnemiesInRange(500).Count > Me.HealthPercent % 65;
            if (!safe)
            {
                result.IsSafe = false;
                return result;
            ...
            {
                if (skillshot.IsDanger(point) && skillshot.IsAboutToHit(500, Me))
                {
        ...
        {
            var currentDashSpeed = 700 + player.MoveSpeed;//At least has to be like this
            var minions = ObjectManager.Get<Obj_AI_Minion>();
            var candidate = new Obj_AI_Minion();
        ...
        public static bool willColide(Skillshot ss, Vector2 from, float speed, Vector2 direction, float radius)
        {
            ...
            var a = ssVel - dashVel;//true direction + speed
        ...
        {
            var currentDashSpeed = 700 + Me.MoveSpeed;//At least has to be like this
            //Get intersection point
            var intersectionPoint = LineIntersectionPoint(Me.Position.To2D(), V2E(Me.Position, jumpOn.Position, 475), ss.Start, ss.End);
            ...
            if (Vector2.DistanceSquared(skillshotPosition, intersectionPoint) <
                (ss.SpellData.Radius + Me.BoundingRadius) && !willColide(skillShot, Me.Position.To2D(), 700f + Me.MoveSpeed, dashDir, Me.BoundingRadius + skillShot.SpellData.Radius))
                return false;
            return true;
        }
        ...
        public static void useEtoSafe(Skillshot skillShot)
        {
            ...
            foreach (var enemy in ObjectManager.Get<Obj_AI_Base>().Where(ob => ob.NetworkId != skillShot.Unit.NetworkId && enemyIsJumpable(ob) && ob.Distance(Me) < E.Range).OrderBy(ene => ene.Distance(Game.CursorPos, true)))
            {
                var pPos = Me.Position.To2D();
        ...
        public static void useWSmart(Skillshot skillShot)
        {
            ...
            if (!W.IsReady() || skillShot.SpellData.Type == SkillShotType.SkillshotCircle || skillShot.SpellData.Type == SkillShotType.SkillshotRing)
                return;
            if (skillShot.IsAboutToHit(WDelay, Me))
            {
        ...
        {
            if (wall.endtime < Game.Time || wall.pointL == null)
                return false;
            var inter = LineIntersectionPoint(vec1.To2D(), vec2.To2D(), wall.pointL.Position.To2D(), wall.pointR.Position.To2D());
        ...
        private static bool HaveStatik => Me.GetBuffCount("ItemStatikShankCharge") == 100;
